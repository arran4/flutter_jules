...
void main() {
  group('SourceProvider', () {
    ...
    test('fetchSources handles pagination', () async {
      ...
      final page1Response = ListSourcesResponse(
        sources: page1Sources,
        nextPageToken: 'token1',
      );

      // Setup page 2
      final page2Sources = <Source>[Source(name: 'source2', id: 'id2')];
      final page2Response = ListSourcesResponse(
        sources: page2Sources,
        nextPageToken: null,
      );

      // Mock sequence
      when(
        mockClient.listSources(pageToken: argThat(isNull, named: 'pageToken')),
      ).thenAnswer((_) async => page1Response);
      when(
        mockClient.listSources(
    ...
    test('ensureSourceAvailable triggers fetch if source missing', () async {
      // Initial state empty
      final initialResponse = ListSourcesResponse(
        sources: [],
        nextPageToken: null,
      );
      when(
