...
class MessageQueueProvider extends ChangeNotifier {
  ...
  Future<void> _migrateQueue() async {
    ...
    for (var i = 0; i < _queue.length; i++) {
        ...
        if (msg.sessionId == 'new_session' && msg.type != QueuedMessageType.sessionCreation) {
           debugPrint("Migrating legacy message ${msg.id} to sessionCreation type");
           try {
               final session = Session(id: '', name: '', prompt: msg.content, sourceContext: SourceContext(source: ''));
               final newMsg = QueuedMessage(
                   ...
                   isDraft: true, // Legacy queue items that weren't sent should be drafts
        ...
        if (_queue[i].isDraft && (_queue[i].queueReason == null || _queue[i].queueReason!.isEmpty)) {
        ...
        if (_queue[i].type == QueuedMessageType.sessionCreation && 
            ...
            (_queue[i].queueReason == null)) {
             ...
             debugPrint("Marking unsent session creation request ${msg.id} as draft.");
