...
class AdvancedSearchBar extends StatefulWidget {
  ...
  final List<FilterToken> availableSuggestions; // All possible filters for autocomplete
...
class _AdvancedSearchBarState extends State<AdvancedSearchBar> {
  ...
  void _onTextChanged() {
    ...
    final isComposingFilter = atIndex != -1 && (atIndex == 0 || text[atIndex - 1] == ' ');
    ...
    if (isComposingFilter) {
       ...
       setState(() {
         _filteredSuggestions = widget.availableSuggestions.where((s) {
            if (widget.activeFilters.any((af) => af.id == s.id && af.type == s.type)) return false;
  ...
  void _tryCommitFilter(String text, int atIndex) {
     ...
     final query = text.substring(atIndex + 1, text.length - 1).toLowerCase(); // remove space
     ...
     final exactMatch = widget.availableSuggestions.firstWhere(
        ...
        orElse: () => FilterToken(id: '', type: FilterType.text, label: '', value: '')
     );
          ...
          child: Material(
            ...
            child: ConstrainedBox(
              ...
              child: ListView.builder(
                ...
                shrinkWrap: true,
                itemCount: _filteredSuggestions.length + 1, // +1 for "Search for @..."
  ...
  void _removeFilter(FilterToken token) {
    final newFilters = List<FilterToken>.from(widget.activeFilters)..remove(token);
    widget.onFiltersChanged(newFilters);
  }
  ...
  Color _getColorForType(FilterType type) {
     switch (type) {
      case FilterType.status: return Colors.blue.shade100;
      case FilterType.source: return Colors.purple.shade100;
      case FilterType.flag: return Colors.green.shade100;
      case FilterType.text: return Colors.grey.shade200;
    }
  }
  ...
  void _toggleSortDirection(SortOption sort) {
    ...
    final newDirection = sort.direction == SortDirection.ascending ? SortDirection.descending : SortDirection.ascending;
  ...
  Widget _buildSortPill(SortOption sort, {bool isDragging = false}) {
     return Padding(
        ...
        child: Tooltip(
           message: "Sort by ${sort.label}. Tap to toggle direction. Drag to reorder.",
           child: InkWell(
             ...
             child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: isDragging ? [BoxShadow(color: Colors.black26, blurRadius: 4, offset:const Offset(0, 2))] : null,
                ),
                child: Row(
  ...
  void _addSort() {
     ...
     final availableFields = SortField.values.where((f) => !existingFields.contains(f)).toList();
     ...
     showMenu<SortField>(
       context: context,
       position: RelativeRect.fromLTRB(offset.dx + size.width - 50, offset.dy + size.height, 0, 0),
       items: availableFields.map((f) => PopupMenuItem(
         value: f,
         child: Text(_getSortFieldLabel(f)),
       )).toList(),
     ).then((value) {
        if (value != null) {
           final newSorts = List<SortOption>.from(widget.activeSorts)..add(SortOption(value, SortDirection.descending));
           widget.onSortsChanged(newSorts);
        }
     });
  }
  ...
  String _getSortFieldLabel(SortField field) {
     switch(field) {
        ...
        case SortField.source: return "Source"; // Repo
            ...
            if (widget.activeFilters.isNotEmpty)
              Wrap(
                ...
                children: widget.activeFilters.map((filter) {
                  return InputChip(
                    label: Text(
                      filter.mode == FilterMode.include ? filter.label : "NOT ${filter.label}",
                      style: TextStyle(
                        decoration: filter.mode == FilterMode.exclude ? TextDecoration.lineThrough : null,
                        color: filter.mode == FilterMode.exclude ? Colors.red : Colors.black87,
                      ),
                    ),
                    avatar: _getIconForType(filter.type),
                    backgroundColor: filter.mode == FilterMode.exclude ? Colors.red.shade100 : _getColorForType(filter.type),
                 ...
                 ...widget.activeSorts.asMap().entries.map((entry) {
                    ...
                    return Draggable<SortOption>(
                      ...
                      child: DragTarget<SortOption>(
                        ...
                        onAccept: (incoming) {
                           final incomingIndex = widget.activeSorts.indexOf(incoming);
