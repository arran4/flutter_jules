...
class SessionMetaPills extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Wrap(
      ...
      children: [
        ...
        if (session.updateTime != null && session.createTime != session.updateTime)
           _buildChip(context,
            avatar: const Icon(Icons.update, size: 16),
             label: "Updated ${DateFormat.yMMMd().add_jm().format(
                DateTime.parse(session.updateTime!).toLocal())}",
             sortField: SortField.updated,
           ),

        // Automation Mode
        if (session.automationMode != null && session.automationMode != AutomationMode.AUTOMATION_MODE_UNSPECIFIED)
          _buildChip(context,
            avatar: const Icon(Icons.smart_toy, size: 16),
            label: "Auto: ${session.automationMode
                .toString()
                .split('.')
                .last
        ...
        if (session.requirePlanApproval != null)
          _buildChip(context,
            ...
            filterToken: FilterToken(
              ...
              label: session.requirePlanApproval! ? 'Approval Required' : 'No Approval', 
              value: session.requirePlanApproval! ? 'approval_required' : 'no_approval'
            ),
          ),
  ...
  Color _getColorForState(SessionState state) {
      ...
      if (state == SessionState.IN_PROGRESS || state == SessionState.PLANNING) return Colors.blue.shade50;
           ...
           items: <PopupMenuEntry>[
             ...
             if (sortField != null) ...[
               ...
               PopupMenuItem(
                 ...
                 onTap: () => onAddSort?.call(SortOption(sortField, SortDirection.ascending)),
               ),
               PopupMenuItem(
                 child: const Row(children: [Icon(Icons.arrow_downward, size: 16), SizedBox(width: 8), Text("Sort Descending")]),
                 onTap: () => onAddSort?.call(SortOption(sortField, SortDirection.descending)),
               ),
             ]
           ]
         );
