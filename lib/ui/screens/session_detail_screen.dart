...
class _SessionDetailScreenState extends State<SessionDetailScreen> {
  ...
  Future<void> _fetchActivities({
    ...
    try {
      ...
      // Add timeout to prevent indefinite hanging
      await _performFetchActivities(force: force, shallow: shallow).timeout(
    ...
    return [
      ...
      if (_session.outputs != null &&
        ...
        IconButton(
          ...
          onPressed: _handleOpenInJules,
        ),
      IconButton(
        icon: const Icon(Icons.data_object),
        tooltip: 'View raw data',
        onPressed: _handleViewRawData,
      ),
        ...
        builder: (context, queueProvider, _) {
          if (queueProvider.isOffline) {
            ...
            return IconButton(
              ...
              tooltip: 'Go Online',
              onPressed: () => _handleGoOnline(queueProvider),
            );
          }
          return IconButton(
  ...
  Future<void> _handleGoOnline(MessageQueueProvider queueProvider) async {
    final auth = Provider.of<AuthProvider>(context, listen: false);
  ...
  Future<void> _handlePopupMenuSelection(String value) async {
    ...
    if (value == 'mark_unread_back') {
      ...
      if (mounted) {
        Navigator.pop(context, const SessionDetailResult(markAsRead: false));
      }
      ...
      showDialog(
        context: context,
        builder: (context) => TagManagementDialog(session: _session),
      );
    } else if (value == 'note_visibility') {
      _toggleNoteVisibility();
    } else if (value == 'view_metadata') {
      if (_session.metadata != null && _session.metadata!.isNotEmpty) {
        showDialog(
          context: context,
          builder: (context) => MetadataViewer(metadata: _session.metadata!),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
  ...
  List<PopupMenuEntry<String>> _buildPopupMenuItems(BuildContext context) {
    ...
    return [
      ...
      if (_session.outputs?.any((o) => o.pullRequest != null) == true)
        const PopupMenuItem(
          value: 'pr_back',
          child: Row(
            children: [
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: _buildSessionDetailActions(),
        ),
        body: Column(
          children: [
            if (_isSending)
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          if (activity.unmappedProps.containsKey(
                            'pendingId',
                          )) ...[
                            TextButton(
                              style: TextButton.styleFrom(
                            ...
                            TextButton(
                              ...
                              onPressed: () {
                                ...
                                final id = activity.id.replaceFirst(
                                  'queued-',
                                  '',
                                );
                                Provider.of<MessageQueueProvider>(
