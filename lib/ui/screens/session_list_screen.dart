...
class _SessionListScreenState extends State<SessionListScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      Future<void> loadFilterAndSessions() async {
        _loadLastFilter(settings);

        final auth = Provider.of<AuthProvider>(context, listen: false);
        if (auth.token == null) return;

        await _refreshSessionsOnStart(settings, auth);
        await _refreshWatchedOrDirty(settings, auth);
  ...
  void _loadLastFilter(SettingsProvider settings) {
  ...
  Future<void> _refreshSessionsOnStart(
  ...
  Future<void> _refreshWatchedOrDirty(
    SettingsProvider settings,
    AuthProvider auth,
  ) async {
    if (!mounted) return;
  ...
  void _fetchSources(AuthProvider auth) {
  ...
  Future<void> _createSession() async {
    ...
    Future<void> performCreate(
      ...
      try {
        ...
        if (e is InvalidTokenException ||
          ...
          if (!isSilent && sessionsToCreate.length == 1) {
            showDialog(
              context: context,
                ...
                actions: [
                  ...
                  TextButton(
                    onPressed: () {
                      ...
                      performCreate(sessionToCreate, isSilent: isSilent);
                    },
            ...
            // For bulk or silent, just notify once? or rely on queue indicators.
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
          ...
          if (errorType != ApiErrorType.unknown) {
            ...
            if (handled) {
              Provider.of<MessageQueueProvider>(
                context,
                listen: false,
              ).addCreateSessionRequest(sessionToCreate, reason: reason);
              if (!isSilent && sessionsToCreate.length == 1) {
                ScaffoldMessenger.of(
                  context,
        ...
        if (!handled) {
          ...
          final msgId = queueProvider.addCreateSessionRequest(
            sessionToCreate,
            reason: 'Creation failed: $e',
            isDraft:
                isSilent, // If silent, mark as draft (with error) to prevent loop
          );
          ...
          if (!isSilent && sessionsToCreate.length == 1) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text("Session creation failed: $e")),
            );
          }
        }
      }
    ...
    if (sessionsToCreate.length > 1 && !result.openNewDialog) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
    ...
    if (result.openNewDialog) {
      ...
      for (final session in sessionsToCreate) {
        // Fire and forget, suppress dialogs
        performCreate(session, isSilent: true);
      }

      // Immediately open new dialog
  ...
  List<Widget> _buildAppBarActions(SettingsProvider settings) {
    return [
      _buildGithubStatusAction(),
      _buildQueueActionRow(settings),
      IconButton(
      ...
      _buildOverflowMenu(settings),
    ];
  }

  Widget _buildGithubStatusAction() {
            ...
            onPressed: () {
              showDialog(
                ...
                builder: (_) => AlertDialog(
                  title: const Text('GitHub Authentication Error'),
                  content: Text(
                    github.authError ??
  ...
  Widget _buildQueueActionRow(SettingsProvider settings) {
    return Consumer<MessageQueueProvider>(
  ...
  Widget _buildOverflowMenu(SettingsProvider settings) {
      ...
      child: Align(
        ...
        child: Consumer<RefreshService>(
          builder: (context, refreshService, _) {
            final next = refreshService.getNextScheduledRefresh();
            String nextText = '';
            ...
            if (_refreshStatus != null) {
              ...
              String type = 'Refresh';
              if (next.schedule.taskType == RefreshTaskType.refresh) {
                final policyName = next.schedule.refreshPolicy?.name ?? '';
      ...
      child: InkWell(
          ...
          if (session.id.startsWith('DRAFT_CREATION_')) {
            ...
            try {
              ...
              if (queueItem.state == QueueState.sending) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Session creation in progress...'),
                  ),
                );
                return;
              }
              if (queueItem.state == QueueState.sent) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Session sent. Waiting for sync...'),
                  ),
                );
                return;
              }
              if (queueItem.state == QueueState.queued) {
                ScaffoldMessenger.of(context).showSnackBar(
        ...
        onLongPress: () async {
          ...
          if (session.id.startsWith('DRAFT_CREATION_')) {
            ...
            if (itemState == QueueState.queued ||
                itemState == QueueState.failed) {
              handled = true;
                ...
                builder: (context) => Column(
                  ...
                  children: [
                    ListTile(
                      ...
                      title: const Text('Edit in New Session'),
                ...
                if (result != null && mounted) {
                  if (result.isDelete) {
                    ...
                    queueProvider.updateCreateSessionRequest(
                      ...
                      result.session,
                      isDraft: true,
                      reason: "User edited queued item",
                    );
                  } else {
                    queueProvider.updateCreateSessionRequest(
                      realId,
                      result.session,
                      isDraft: false,
                      reason: "User edited and re-queued",
                    );
                  }
                }
            ...
            children: [
              Row(
                children: [
                      ...
                      children: [
                        ...
                        if (session.prStatus != null &&
                          ...
                          _buildPill(
                            ...
                            backgroundColor: session.prStatus == 'Merged'
                                ? Colors.green
                                : Colors.red,
                            textColor: Colors.white,
                        ...
                        Expanded(
                          child: LayoutBuilder(
                            builder: (context, constraints) {
                              ...
                              return PopupText(
                                (session.title ?? session.prompt).replaceAll(
                                  '\n',
                                  ' ',
                                ),
                                maxLines: maxLines,
                  ...
                  if ((session.prStatus == null || session.prStatus!.isEmpty) &&
                      (session.diffUrl != null ||
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<SessionProvider>(
      builder: (context, sessionProvider, child) {
        ...
        final draftSessions = _buildDraftSessions(queueProvider);
  ...
  void _openNextSession(Session closedSession) {
    ...
    final index = _displayItems.indexWhere(
      (item) => item.data.id == closedSession.id,
    );
  ...
  Future<void> _fetchAndShowRawData(Session session) async {
    final auth = Provider.of<AuthProvider>(context, listen: false);
    final cacheService = Provider.of<CacheService>(context, listen: false);
    ...
    try {
      ...
      if (cachedDetails != null &&
        ...
        final fetchedActivities = await auth.client.listActivities(
          session.name,
        );
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Failed to load raw data: $e')));
      }
    }
  }
  ...
  void _showTileMenu(
    ...
    showMenu(
      ...
      if (value == 'source' && session.sourceContext?.source != null) {
        final sourceProvider = Provider.of<SourceProvider>(
          context,
          listen: false,
        );
